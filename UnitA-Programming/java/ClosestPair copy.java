...
public class ClosestPair {
     ...
     */
     public point[] parses(List<String> fileData){
        int sizeList = fileData.size();
        point[] pointList = new point[sizeList];
    ...
    public double distPoints(point[] point, int a, int b){
        return Math.sqrt(((point[a].x - point[b].x)*(point[a].x- point[b].x)) 
    ...
    public double[] bForce(point[] points, int n){
    ...
    public double[] twoClosestPairs(point[] points, int n){
        // check base case
        if (n <=3){
        ...
        // find median
        int midPoint = n/2;
        double med = points[midPoint].x;
        // make two new arrays
        point[] pListLow = new point[midPoint];
        point[] pListHigh = new point[(n-midPoint)];
        for(int i = 0; i < midPoint; i++){
            pListLow[i] = points[i];
        }
        for(int i = midPoint; i < n; i++){
            pListHigh[(i-midPoint)] = points[i];
        }
        ...
        point[] runwayPoints = new point[n];
        int index = 0;
        for (int i = midPoint; i >= 0 ; i--){
            if (points[i].x >= runwayStart && points[i].x <= runwayEnd){
                runwayPoints[index] = points[i];
                index++;
            };
            if (points[i].x < runwayStart || points[i].x > runwayEnd){
                break;
        ...
        point[] runwayPoints1 = new point[index];
        for(int i = 0; i < index; i++){
            runwayPoints1[i] = runwayPoints[i];
        }
    ...
    public double[] compute(List<String> fileData) {
        ...
        point[] nfileData = parses(fileData);
